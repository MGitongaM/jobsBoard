  ...
  return (
    <section className="bg-jobsBg1 min-h-96 px-2 py-10">
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-1 md:gap-10 text-center">
        {jobTypes.map((job) => (
          <Link
            href={`/jobs?q=${job.title}`}
            key={job.title}
            className="p-4 flex flex-col items-center border border-amber-100 rounded-md hover:bg-amber-100 hover:rounded-md hover:shadow-amber-400 hover:shadow-md"
          >
            <div>{job.logo}</div>
            <p className="text-base lg:text-xl font-bold">{job.title}</p>
          </Link>
        ))}
      </div>
